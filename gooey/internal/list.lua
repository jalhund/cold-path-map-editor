...
local function handle_input(list, action_id, action, click_fn)
	...
	if over_stencil and (scroll_up or scroll_down) then
		...
		if list.horizontal then
			...
			list.scroll_pos.y = list.scroll_pos.y + ((scroll_up and 3 or -3) * list.scroll_speed)
		end
...
function M.dynamic(list_id, stencil_id, item_id, data, action_id, action, config, fn, refresh_fn, max_count_size)
	...
	if not list.items or #list.items ~= #data then
		...
		if list.items then
			for k, v in pairs(list.items) do
				gui.delete_node(list.items[k].root)
			end
		end
		list.items = {}
		...
		if list.horizontal then
			...
			item_count = max_count_size or #data
